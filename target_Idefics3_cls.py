...
@torch.no_grad()
def generate_response_llava(model, processor, image, query):
    ...
    if isinstance(image, torch.Tensor):
        ...
        pil_image = to_pil_image(image.cpu().float().clamp(0, 1))
    ...
    with open(args.input_text_path, 'r', encoding='utf-8') as fin, open(args.output_path, file_mode, encoding='utf-8') as fout:
        for line_no, line in enumerate(tqdm(fin)):
            ...
            while True:
                ...
                response = generate_response_llava(model, processor, image_tensor, instruction)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="../Idefics3-8B-Llama3")
