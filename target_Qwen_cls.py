...
@torch.no_grad()
def generate_response_qwen(model, processor, image_tensor, query):
    ...
    image_inputs = [item["image"] for m in messages for item in m["content"] if item["type"] == "image"]
    video_inputs = None
...
def main(args):
    ...
    try:
        from transformers import Qwen2VLImageProcessor as SlowImageProcessor
    except Exception:
        from transformers import Qwen2_5_VLImageProcessor as SlowImageProcessor

    processor = AutoProcessor.from_pretrained(
        args.model_name,
        trust_remote_code=True
    )
    processor.image_processor = SlowImageProcessor.from_pretrained(
    ...
    with open(args.input_text_path, 'r', encoding='utf-8') as fin, open(args.output_path, file_mode, encoding='utf-8') as fout:
        for line_no, line in enumerate(tqdm(fin)):
            ...
            while True:
                ...
                response = generate_response_qwen(model, processor, image_tensor, instruction)
